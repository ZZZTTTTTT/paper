...
for file_name in file_names:
    ...
    with open(fileob, 'r', encoding='utf-8') as f:
        ...
        json_file_to_df.insert(loc=3,column='kpi_id',value=np.repeat(filename_info[2],json_file_to_df.shape[0]))
...
#               timestamp deviceCode point_name kpi_id  \
...
#合并两个表
df_merged=pd.merge(df_json, df_kpi, on=['deviceCode','point_name','kpi_id'], how='left')
df_merged.to_csv('./result.csv',encoding="utf_8_sig",mode="w")
# print(df_merged)因为剔除了kpi项所以有两万多空的数据
