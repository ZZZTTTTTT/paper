...
class LSTM_Demo:
    ...
    def split_train_test(self, reframed):
        ...
        self.n_obs = self.n_seq * self.n_features
        # 有60=(12*(3+2))列数据
        train_X, train_y = train[:, :-self.n_obs], train[:, -self.n_obs:]
        test_X, test_y = test[:, :-self.n_obs], test[:, -self.n_obs:]
    ...
    def do_train(self):
        ...
        # 拟合网络
        self.history = self.model.fit(train_X, train_y, epochs=125, batch_size=self.n_batch,
    ...
    def do_ED_train(self):
        ...
        self.model = Sequential()
        self.model.add(
            LSTM(200, activation='relu', input_shape=(train_X.shape[1], train_X.shape[2]),
    ...
    def do_ED_Attention_train(self):
        ...
        # lstm输入形状[batch_size, time_step, input_size]
        x = Input(shape=(train_X.shape[1], train_X.shape[2]))
        lstm_out = LSTM(400, return_sequences=True, activation='relu')(x)
        ...
        lstm_out = LSTM(320,activation='relu',  return_sequences=False)(lstm_out)
        lstm_out = Lambda(lambda x: K.repeat_elements(x[:, None, :], train_y.shape[1], axis=1))(lstm_out)

        lstm_out = TimeDistributed(Dense(200, activation='relu'))(lstm_out)
        # lstm_out = Dropout(0.3)(lstm_out)
        ...
        self.n_batch=256
        # fit network
        self.history = self.model.fit(train_X, train_y, epochs=50, batch_size=self.n_batch, verbose=1,
                                      validation_data=(test_X, test_y), shuffle=False)
        print(self.history.history)
        print(self.history.history['loss'])
        print(self.history.history['val_loss'])
